    ...
    public partial class FlightPlanner : MyUserControl, IDeactivate, IActivate
    {
        ...
        private void CMB_serialport_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (Utilities.Settings.Instance[_connectionControl.CMB_serialport.Text + "_BAUD"] != null)
                {
                    _connectionControl.CMB_baudrate.Text =
                        Utilities.Settings.Instance[_connectionControl.CMB_serialport.Text + "_BAUD"];
                }
            }
            catch
            {
        ...
        void comPort_MavChanged(object sender, EventArgs e)
        {
            ...
            instance.MyView.AddScreen(new MainSwitcher.Screen("FlightPlanner", this, true));
            instance.MyView.ShowScreen("FlightPlanner");
        ...
        private void config(bool write)
        {
            if (write)
            {
                ...
                //Utilities.Settings.Instance["fpcoordmouse"] = coords1.System;
            }
            else
            {
                foreach (string key in Utilities.Settings.Instance.Keys)
                {
                    switch (key)
                    {
                        ...
                        case "fpminaltwarning":
                            ...
                            break;
                            //case "fpcoordmouse":
                            //    coords1.System = "" + Utilities.Settings.Instance[key];
                            break;
                        default:
                            break;
                    }
        ...
        */
        ...
        private void comboBoxMapType_SelectedValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                MainV2.mapP = (GMapProvider)comboBoxMapType.SelectedItem;
            }
            catch (Exception ex)
            {
                log.Error(ex);
...
Column 1: Field type (RALLY is the only one at the moment -- may have RALLY_LAND in the future)
 ...
 Column 7: Flags (just 0 for now, also future use).
        ...
        private void menuTakeoff_Click(object sender, EventArgs e)
        {
            ...
            {
                CustomMessageBox.Show("กรุณาเลือกฟาร์มและกิจกรรมการบินที่ต้องการ");
            }
        }
        ...
        public void doConnect(MAVLinkInterface comPort, string portname, string baud)
        {
            ...
            try
            {
                ...
                if (Utilities.Settings.Instance.GetBoolean("CHK_resetapmonconnect") == true)
                {
                ...
                try
                {
                    Directory.CreateDirectory(Utilities.Settings.Instance.LogDir);
                    lock (this)
                    {
                        // create log names
                        var dt = DateTime.Now.ToString("yyyy-MM-dd HH-mm-ss");
                        var tlog = Utilities.Settings.Instance.LogDir + Path.DirectorySeparatorChar +
                                   dt + ".tlog";
                        var rlog = Utilities.Settings.Instance.LogDir + Path.DirectorySeparatorChar +
                                   dt + ".rlog";
                        ...
                        while (File.Exists(tlog))
                        {
                            ...
                            tlog = Utilities.Settings.Instance.LogDir + Path.DirectorySeparatorChar +
                                   dt + ".tlog";
                            rlog = Utilities.Settings.Instance.LogDir + Path.DirectorySeparatorChar +
                                   dt + ".rlog";
                        }
                ...
                Utilities.Settings.Instance[_connectionControl.CMB_serialport.Text + "_BAUD"] = _connectionControl.CMB_baudrate.Text;
                ...
                if (Utilities.Settings.Instance.GetBoolean("loadwpsonconnect") == true)
                {
        ...
        public void doDisconnect(MAVLinkInterface comPort)
        {
            ...
            try
            {
                System.Threading.ThreadPool.QueueUserWorkItem((WaitCallback)delegate
                {
                    try
                    {
                        MissionPlanner.Log.LogSort.SortLogs(Directory.GetFiles(Utilities.Settings.Instance.LogDir, "*.tlog"));
                    }
                    catch
                    {
                    }
            ...
            try
            {
                ...
                foreach (var serial in serials)
                {
                    if (serial.Contains(comPort.MAV.SerialString.Substring(comPort.MAV.SerialString.Length - 26, 26)) &&
                        !Utilities.Settings.Instance.ContainsKey(comPort.MAV.SerialString.Replace(" ", "")))
                    {
                        CustomMessageBox.Show(
                        ...
                        Utilities.Settings.Instance[comPort.MAV.SerialString.Replace(" ", "")] = true.ToString();
                    }
                }
            }
            catch
